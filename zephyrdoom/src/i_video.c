 ...
 * When the screen is not visible, don't render the screen.
 */
const boolean screenvisible = true;

/* If true, we display dots at the bottom of the screen to indicate FPS. */
...
void I_SetPalette(byte *doompalette)
{
     ...
     *
     * TODO
     * Do conversion right before transferring to save memory.
     */
    for (i = 0; i < 256; ++i)
    {
        /*
 ...
 *     for (i = 0; i < 256; ++i)
 *     {
 *         diff = (r - palette[i].r) * (r - palette[i].r) + (g - palette[i].g) * (g - palette[i].g) + (b - palette[i].b) * (b - palette[i].b);
 *
 *         if (diff < best_diff)
 *         {
 *             best = i;
