...
#define DLINECOLOR  0x001F		// 16-bit RGB value for pixelated diagonal line (5-6-5 RGB)
...
int draw_pixel(const struct device *screen, uint8_t x, uint8_t y, uint32_t color)
{
	uint8_t *buf;
...
int ili_do_stuff(void)
{
	...
	for (int row = 0; row < 10; row++) {
		for (int col = 0; col < 10; col++) {
			draw_pixel(screen, 100 + row, 100 + col, 0x07E0); // Green (5-6-5 RGB)
		}
	}
