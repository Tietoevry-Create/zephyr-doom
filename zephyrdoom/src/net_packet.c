...
char *NET_ReadString(net_packet_t *packet)
{
    ...
    /*
     * packet->data[packet->pos] == '\0'
     * We have reached a terminating NULL.
     * Skip past this NULL and continue reading immediately after it.
     */
    ++packet->pos;
...
char *NET_ReadSafeString(net_packet_t *packet)
{
    ...
    /*
     * w is always <= r
     * We never produce a longer string than the original.
     */
    w = result;
    for (r = result; *r != '\0'; ++r)
    {
        /*
         * TODO
         * This is a very naive way of producing a safe string.
         * Only ASCII characters are allowed.
         * Probably this should really support UTF-8 characters as well.
         */
        if (isprint(*r) || *r == '\n')
        {
            *w = *r;
