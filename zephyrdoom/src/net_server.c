...
typedef struct
{
     ...
     * This is used to determine the controller (the oldest client).
     */
...
{
    /*
     * Check that the client is properly connected, i.e. not in the
...
static void NET_SV_AdvanceWindow(void)
{
     ...
     *     if (!should_advance)
     *     {
     *         // The first tic is not complete, i.e. we have not
                            ...
                            net_addr_t *addr)
{
     ...
     * if (!NET_ReadConnectData(packet, &data) || !D_ValidGameMode(data.gamemission, data.gamemode) || data.max_players > NET_MAXPLAYERS)
     * {
     *     return;
     * }
     *
     ...
     *
     * // TODO
     * // Add server option to allow rejecting clients which set
...
static void NET_SV_ParseResendRequest(net_packet_t *packet, net_client_t *client)
{
     ...
     *         // but then again this could be a spoofed packet.
     *         // Just ignore it.
     *         return;
     *     }
     * }
