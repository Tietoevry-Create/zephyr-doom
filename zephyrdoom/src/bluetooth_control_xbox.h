...
void convert_uuid128_to_uuid16_services(struct bt_gatt_dm *dm)
{
	...
	{
		if (!bt_uuid_cmp(dm->attrs[i].uuid, BT_UUID_DECLARE_128(BT_UUID_128_ENCODE(BT_UUID_HIDS_INFO_VAL, 0, 0, 0, 0))))
		{
			dm->attrs[i].uuid->type = BT_UUID_TYPE_16;
			BT_UUID_16(dm->attrs[i].uuid)->val = BT_UUID_HIDS_INFO_VAL;
		}

		else if (!bt_uuid_cmp(dm->attrs[i].uuid, BT_UUID_DECLARE_128(BT_UUID_128_ENCODE(BT_UUID_HIDS_CTRL_POINT_VAL, 0, 0, 0, 0))))
		{
			dm->attrs[i].uuid->type = BT_UUID_TYPE_16;
			BT_UUID_16(dm->attrs[i].uuid)->val = BT_UUID_HIDS_CTRL_POINT_VAL;
		}

		else if (!bt_uuid_cmp(dm->attrs[i].uuid, BT_UUID_DECLARE_128(BT_UUID_128_ENCODE(BT_UUID_HIDS_REPORT_MAP_VAL, 0, 0, 0, 0))))
		{
			dm->attrs[i].uuid->type = BT_UUID_TYPE_16;
			BT_UUID_16(dm->attrs[i].uuid)->val = BT_UUID_HIDS_REPORT_MAP_VAL;
		}

		else if (!bt_uuid_cmp(dm->attrs[i].uuid, BT_UUID_DECLARE_128(BT_UUID_128_ENCODE(BT_UUID_HIDS_REPORT_VAL, 0, 0, 0, 0))))
		{
			dm->attrs[i].uuid->type = BT_UUID_TYPE_16;
			BT_UUID_16(dm->attrs[i].uuid)->val = BT_UUID_HIDS_REPORT_VAL;
			BT_UUID_16(dm->attrs[11].uuid)->val = BT_UUID_HIDS_REPORT_VAL; //hardcoded for XBOX
		}
				   ...
				   void *context)
{
	...
	printk("\nFor XBOX controllers, you need to manually exit pairing mode or wait for the timeout\n\n");

	convert_uuid128_to_uuid16_services(dm);	 						//need convert some UUID128 into UUID16 (for XBOX Controller)
			     ...
			     const uint8_t *data)
{
	...
	e.data1 = (button_A << 4) | (button_B << 5) | (button_X << 2) | (button_Y << 3);

	int32_t joyX = (data[0] << 8 | data[1]);
	int32_t joyY = (data[2] << 8 | data[3]);
