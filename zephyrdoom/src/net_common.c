...
static void NET_Conn_ParseReliableACK(net_connection_t *conn, net_packet_t *packet)
{
    ...
    if (seq == (unsigned int)((conn->reliable_packets->seq + 1) & 0xff))
    {
        ...
        /* Discard it, then. Unlink from the list. */
                                       ...
                                       net_packet_t *packet)
{
     ...
     *
     * This is braindead. It would be much more sensible to
...
{
    if (conn->state != NET_CONN_STATE_DISCONNECTED && conn->state != NET_CONN_STATE_DISCONNECTING && conn->state != NET_CONN_STATE_DISCONNECTED_SLEEP)
    {
...
void NET_Conn_Run(net_connection_t *conn)
{
    ...
    if (conn->state == NET_CONN_STATE_CONNECTED)
    {
         ...
         * list timed out.
         *
         * This is braindead, we have a fixed time of one second.
         */
        if (conn->reliable_packets != NULL && (conn->reliable_packets->last_send_time < 0 || nowtime - conn->reliable_packets->last_send_time > 1000))
        {
