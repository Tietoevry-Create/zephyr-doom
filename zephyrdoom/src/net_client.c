                                    ...
                                    ticcmd_t *ticcmds)
{
     ...
     *         // Low level filter
     *
     *         average_latency = (fixed_t)((average_latency * 0.9) + (latency * FRACUNIT * 0.1));
     *     }
     * }
     *
...
static void NET_CL_Shutdown(void)
{
     ...
     *
     *     // Shut down network module, etc. here.
     * }
     */
}
...
static void NET_CL_ParseWaitingData(net_packet_t *packet)
{
     ...
     * }
     *
     * if (wait_data.num_players > wait_data.max_players || wait_data.ready_players > wait_data.num_players || wait_data.max_players > NET_MAXPLAYERS)
     * {
     *     // Insane data
     *     return;
     * }
     *
     * if ((wait_data.consoleplayer >= 0 && drone) || (wait_data.consoleplayer < 0 && !drone) || (wait_data.consoleplayer >= wait_data.num_players))
     * {
...
static void NET_CL_ParseGameData(net_packet_t *packet)
{
     ...
     * }
     *
     * // Has this been received out of sequence, i.e. have we not received
...
/* "Run" the client code, check for new packets, send packets as needed. */
void NET_CL_Run(void)
{
     ...
     *
     * if (client_connection.state == NET_CONN_STATE_DISCONNECTED || client_connection.state == NET_CONN_STATE_DISCONNECTED_SLEEP)
     * {
     *     NET_CL_Disconnected();
     *
     *     NET_CL_Shutdown();
...
boolean NET_CL_Connect(net_addr_t *addr, net_connect_data_t *data)
{
     ...
     *     NET_CL_Run();
     *
     *     // Run the server, just incase we are doing a loopback connect
     *     NET_SV_Run();
     *
     *     // Don't hog the CPU
     *     I_Sleep(1);
...
void NET_CL_Disconnect(void)
{
     ...
     * start_time = I_GetTimeMS();
     *
     * while (client_connection.state != NET_CONN_STATE_DISCONNECTED && client_connection.state != NET_CONN_STATE_DISCONNECTED_SLEEP)
     * {
