...
boolean ST_Responder(event_t *ev)
{
     ...
     */
    if (ev->type == ev_keyup && ((ev->data1 & 0xf000) == AM_MSGHEADER))
    {
        switch (ev->data1)
        {
        case AM_MSGENTERED:
    ...
    else if (ev->type == ev_keydown)
    {
        if (!netgame && gameskill != sk_nightmare)
        {
            ...
            else if ((logical_gamemission == doom && cht_CheckCheat(&cheat_noclip, ev->data2)) || (logical_gamemission != doom && cht_CheckCheat(&cheat_commercial_noclip, ev->data2)))
            {
                /*
                 * Noclip cheat.
...
void ST_updateFaceWidget(void)
{
    ...
    if (priority < 8)
    {
        if (plyr->damagecount && plyr->attacker && plyr->attacker != plyr->mo)
        {
             ...
             * NRFD-TODO
             * To fix or not?
             * https://doom.fandom.com/wiki/Ouch_face
             */
...
void ST_doPaletteStuff(void)
{
     ...
     * // as though the player is being covered in goo by an attacking flemoid.
     *
