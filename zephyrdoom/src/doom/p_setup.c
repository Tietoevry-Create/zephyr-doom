...
sector_t *SegFrontSector(seg_t *seg)
{
    /*
     * NRFD-TODO
     * Optimize. Use numbers instead of pointers.
     */
...
/*
 * NRFD-TODO
 * Consider returning data instead of pointer.
 */
...
void P_LoadLineDefs(int lump)
{
    ...
    for (i = 0; i < numlines; i++, mld++, ld++)
    {
        ...
        if (v1_num > numvertexes)
            I_Error("P_LoadLineDefs v1 %d > %d", v1_num, numvertexes);
        if (v2_num > numvertexes)
            I_Error("P_LoadLineDefs v2 %d > %d", v2_num, numvertexes);
...
fixed_t *LineBBox(line_t *ld)
{
    ...
    /*
     * // TODO
     * if (ld->v1 >= &vertexes[numvertexes])
     *     I_Error("LineBBox v1 overflow %X vs %X", (unsigned int)ld->v1, (unsigned int)vertexes);
     * if (ld->v1 < &vertexes[0])
     *     I_Error("LineBBox v1 underflow %X vs %X", (unsigned int)ld->v1, (unsigned int)vertexes);
     * if (ld->v2 >= &vertexes[numvertexes])
     *     I_Error("LineBBox v2 overflow %X vs %X", (unsigned int)ld->v2, (unsigned int)vertexes);
     * if (ld->v2 < &vertexes[0])
     *     I_Error("LineBBox v2 underflow %X vs %X", (unsigned int)ld->v2, (unsigned int)vertexes);
     */
...
void P_LoadBlockMap(int lump)
{
    ...
    /*
     * // NRFD-EXCLUDE
     * // Don't need to do endian shift here.
...
static void PadRejectArray(byte *array, unsigned int len)
{
    ...
    if (len > sizeof(rejectpad))
    {
         ...
         * (or 0xff, if specified on command line).
         */
