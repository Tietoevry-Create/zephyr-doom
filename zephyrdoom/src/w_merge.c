...
static sprite_frame_t *FindSpriteFrame(char *name, int frame)
{
    ...
    /* Not found in list. Need to add to the list. Grow list. */
...
static boolean SpriteLumpNeeded(lumpinfo_t *lump)
{
    ...
    /* Second frame if any. No second frame. */
    if (lump->name[6] == '\0')
        return false;
...
static void GenerateSpriteList(void)
{
    ...
    /* Add all sprites from the PWAD (replaces IWAD sprites) */
